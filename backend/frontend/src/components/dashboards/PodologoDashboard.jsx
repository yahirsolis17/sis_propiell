...
const PodologoDashboard = ({ cardAnimation }) => {
  ...
  useEffect(() => {
    ...
    const fetchCitas = async () => {
      try {
        ...
        // Citas próximas = citas pendientes (estado "P") de este podólogo
        ...
        console.error("Error al cargar citas del podólogo:", err);
        ...
        setError(
          "Error al cargar las próximas citas. Intenta de nuevo en unos momentos."
        );
      } finally {
  ...
  const updateCitaInList = (citaId) => {
    // Igual que en DermatologoDashboard: al atender/cambiar estado,
    // la quitamos de "próximas"
      ...
      <animated.div style={cardAnimation} className="dashboard-card">
        {loading ? (
          ...
          <section className="citas-section">
            <h3 className="mb-3">
              Próximas citas (Podología) ({citas.length})
            </h3>
